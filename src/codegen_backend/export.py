...
def export_generic_c(
    ...
    header_lines = [
        ...
        f" *   truncate_weights_after: {truncate_weights_after!r}",
        f" *   temp_allocation_threshold: {temp_allocation_threshold!r}",
        f" *   variable_dim_inputs: {variable_dim_inputs!r}",
        f" *   variable_dim_outputs: {variable_dim_outputs!r}",
        " */",
        "",
    ]
    header_text = "\n".join(header_lines)
    ...
    final_source = header_text + source.rstrip() + "\n\n" + wrapper
