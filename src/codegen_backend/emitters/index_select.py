...
def _write_index_select_kernel(
    node_index: int,
    op_spec: _OpSpec,
    ...
    index_select_template = get_template_env().get_template(
        "index_select_kernel.c.j2"
    )
    ...
    input_indices = [
        "idx" if dim == select_dim else f"i{dim}"
    ...
    rendered = index_select_template.render(
        signature=signature,
...
class IndexSelectEmitter(KindEmitterBase):
    def emit(self, req: KernelEmitRequest) -> List[str]:
        op_spec = req.op_spec
        dtype = req.dtype
        if op_spec is None or dtype is None:
            raise CodegenBackendError("index_select requires op spec and dtype")
        return _write_index_select_kernel(
            req.node_index,
            op_spec,
            req.input_shapes[0],
            req.input_shapes[1],
