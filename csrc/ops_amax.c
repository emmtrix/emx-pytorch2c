...
static int update_max(float *acc, float value, int *initialized) {
    if (!*initialized) {
        *acc = value;
        *initialized = 1;
        return isnan(value);
                            ...
                            for (int64_t i6 = 0; i6 < sizes[6]; ++i6) {
                                for (int64_t i7 = 0; i7 < sizes[7]; ++i7) {
                                    int64_t a_offset = i0 * a_strides[0]
                                        ...
                                        + i6 * a_strides[6]
                                        + i7 * a_strides[7];
                                    if (update_max(&acc, a_data[a_offset], &initialized)) {
                                        out_data[0] = acc;
                                        return;
                                    }
                                }
...
int ref_run_amax(const RefOpCall *call, char *err_msg, size_t err_cap) {
    if (call->n_inputs != 1 || call->n_outputs != 1) {
        write_errorf(err_msg, err_cap, "amax", "expects exactly 1 input and 1 output");
        return 1;
    }
    ...
    if (a->dtype != REF_F32 || out->dtype != REF_F32) {
        write_errorf(err_msg, err_cap, "amax", "supports only REF_F32 dtype");
        return 2;
    }
    if (out->ndim != 0) {
        write_errorf(err_msg, err_cap, "amax", "requires output to be a scalar tensor");
        return 3;
    }
    if (a->ndim > 8) {
        write_errorf(err_msg, err_cap, "amax", "supports at most 8 dimensions");
        return 4;
    }
