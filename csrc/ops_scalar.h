...
static inline float ref_scalar_angle(float a) {
    if (isnan(a)) {
        return a;
    }
...
static inline float ref_scalar_conj(float a) {
    return a;
}

static inline float ref_scalar_conj_physical(float a) {
    return a;
}

static inline float ref_scalar_deg2rad(float a) {
    return a * (REF_PI_F / 180.0f);
}

static inline float ref_scalar_digamma(float x) {
    if (isnan(x) || isinf(x)) {
        return x;
    }
    if (x <= 0.0f) {
        ...
        return ref_scalar_digamma(1.0f - x) - REF_PI_F / tanf(REF_PI_F * x);
    }
    float result = 0.0f;
    while (x < 6.0f) {
        result -= 1.0f / x;
...
static inline float ref_scalar_erfinv(float x) {
    if (isnan(x)) {
        return x;
    }
    if (x <= -1.0f) {
...
static inline float ref_scalar_exp2(float a) {
    return exp2f(a);
}

static inline float ref_scalar_frac(float a) {
    return a - floorf(a);
}

static inline float ref_scalar_i0(float x) {
    float ax = fabsf(x);
    if (ax < 3.75f) {
        float y = x / 3.75f;
        y *= y;
...
static inline float ref_scalar_lgamma(float a) {
    return lgammaf(a);
}

static inline float ref_scalar_logit(float a) {
    if (isnan(a)) {
        return a;
    }
    if (a == 0.0f) {
...
static inline float ref_scalar_nan_to_num(float a) {
    if (isnan(a)) {
        return 0.0f;
    }
    if (isinf(a)) {
...
static inline float ref_scalar_positive(float a) {
    return a;
}

static inline float ref_scalar_rad2deg(float a) {
    return a * (180.0f / REF_PI_F);
}

static inline float ref_scalar_real(float a) {
    return a;
}

static inline float ref_scalar_sgn(float a) {
    if (isnan(a)) {
        return 0.0f;
    }
    if (a > 0.0f) {
...
static inline float ref_scalar_sinc(float a) {
    if (a == 0.0f) {
        return 1.0f;
    }
    float x = REF_PI_F * a;
...
static inline float ref_scalar_square(float a) {
    return a * a;
}
